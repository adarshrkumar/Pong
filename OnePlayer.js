class OnePlayer extends SimpleScene {
  constructor() {
    super("OnePlayer");
    this.size = 4
    this.sizeChange = 1
  ...
  create() {
    ...
    // add circle
    this.ball = this.add.circle((window.innerWidth-16)/2, (window.innerHeight)/2, this.ballSize, 0xFFFFFF);
    // add rectangle
  ...
  update() {
    ...
    if (this.up.isPressed()) {
      this.player1.y -= 5;
      //      player1Stats[position][y] = this.player1.y
    }
    ...
    if (this.upArrowKey.isPressed()) {
      this.player1.y -= 5;
      //      player2Stats[position][y] = this.player1.y
    }
    if (this.downArrowKey.isPressed()) {
      this.player1.y += 5;
      //      player2Stats[position][y] = this.player1.y
    }
    ...
    if (this.ball.x + (this.ballSize/2) < 0) {
      lostPlayer = 1
      this.scene.start("OneScreen");
      this.ballMvSclFactor = this.orogBallMvSclFactor
    }
  ...
  switchBallDir(player) {
    ...
    if (this.ballHorizDir === 'L'/* && this.isInArea !== true*/) {
      this.ballHorizDir = 'R'
      if (this.ballMvSclFactor < 5 && player !== 2) {
